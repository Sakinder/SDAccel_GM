namespace eval ZQ {
  namespace eval VIV {
    ...
    # hidden function: official not supported
	  ...
	  puts "------*********************************----------"
		set_property STEPS.OPT_DESIGN.TCL.PRE ${ZQ::SET_PATH}/xpr_preopt.tcl [get_runs impl_1]
		set_property STEPS.OPT_DESIGN.TCL.POST ${ZQ::SET_PATH}/xpr_postopt.tcl [get_runs impl_1]
		set_property STEPS.ROUTE_DESIGN.TCL.POST ${ZQ::SET_PATH}/xpr_postroute.tcl [get_runs impl_1]
puts "------*********************************----------"
      close $fp_w
    } 
    # -----------------------------------------
    ...
    # hidden function: official not supported
      if {[file exists $ZQ::HDL_PATH]} {
        ...
        } elseif {[file exists $ZQ::HDL_PATH/${ZQ::VPROJ_NAME}.v]} {
          #Verilog
          set_property top ${ZQ::VPROJ_NAME} [current_fileset]
          #overwrite bd_import toplevel_settings
          puts "Info:(ZQ) Use new Toplevel name:"
          set ZQ::PR_TOPLEVELNAME ${ZQ::VPROJ_NAME}
          puts "ZQ::PR_TOPLEVELNAME:$ZQ::PR_TOPLEVELNAME"
        }
        # set attributes
    ...
    # -----------------------------------------
    # -----------------------------------------
    # --creat_new_blockdesign: create empty block design with zynq initialisation: fsys,msys,xcl_design,zusys
   ...
   # -----------------------------------------
    ...
    proc write_viv_cfgmem {} {
      ...
      if {!$ZQ::IS_xcl_design && !$ZQ::IS_ZUSYS} {
